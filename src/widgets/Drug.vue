...
const requestAdd = async () => {
  ...
  if (!dialog) throw new Error('Cannot open the dialog because it doesn\'t exist!')

  try {
    const number = await dialog.requestDialogResult({ fromWidget: 'DRUG' })
    add(number)
  } catch (err) {
